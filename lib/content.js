...
export function validateUpdates(page, body) {
  ...
  for (const [field, value] of Object.entries(body)) {
    ...
    if (field === 'profile_image' && !(cleaned.startsWith('/') || cleaned.startsWith('https://'))) {
      return { error: 'Profile image must be an uploaded image URL.' };
    }
...
export function validateItem(name, body, { partial = false } = {}) {
  ...
  for (const field of config.fields) {
    ...
    // A `lines` field is a list in a textarea: drop blank lines and stray
    // indentation so the stored text always round-trips to the same list.
    if (field.type === 'lines') {
      cleaned = cleaned
        .split('\n')
        .map((line) => line.trim())
    ...
    if (field.type === 'select' && !field.options.some((o) => o.value === cleaned)) {
      return { error: `${field.label} must be one of the listed options.` };
    }
